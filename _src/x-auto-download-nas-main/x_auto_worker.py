    ...
    if parsed is not None:
        ...
        if isinstance(raw_items, list):
            ...
            for item in raw_items:
                ...
                if "x.com" not in domain or not name or not value:
                    continue
                if name == "twid":
                    user_id = value.removeprefix("u%3D")
                same_site_raw = str(item.get("sameSite") or item.get("same_site") or "Lax").lower()
                same_site = {"strict": "Strict", "lax": "Lax", "none": "None", "no_restriction": "None"}.get(
                    same_site_raw,
                    "Lax",
                )
                cookie = {
                ...
                expires = item.get("expires", item.get("expirationDate", item.get("expiration_date")))
                if expires:
                    cookie["expires"] = int(float(expires))
                elif name in {"auth_token", "ct0", "twid"}:
                    cookie["expires"] = now + 86400 * 180
                cookies.append(cookie)
        return cookies, user_id, "json"
...
def write_cookie_file(path: Path, cookie_text: str) -> None:
    ...
    if "# Netscape HTTP Cookie File" in text or "\t" in text:
        ...
        return
    cookies, _user_id, source_format = parse_cookie_text(text)
    if source_format == "json" and cookies:
        expires = int(time.time()) + 86400 * 180
        ...
        for cookie in cookies:
            ...
            expiry = int(cookie.get("expires") or expires)
            lines.append(
                f"{domain}\t{include_subdomains}\t{cookie.get('path') or '/'}\t{secure}\t{expiry}\t{cookie['name']}\t{cookie['value']}"
            )
        path.write_text("\n".join(lines) + "\n", encoding="utf-8")
        return
    expires = int(time.time()) + 86400 * 180
...
def load_cookie_source(config: dict[str, Any]) -> tuple[str, str]:
    env_name = str(config.get("cookie_env") or "X_COOKIE")
    env_value = os.environ.get(env_name, "").strip()
    if env_value:
        return env_value, f"env:{env_name}"

    env_file_name = str(config.get("cookie_file_env") or "X_COOKIE_FILE")
    env_file = os.environ.get(env_file_name, "").strip()
    candidates = [env_file, str(config.get("cookie_file") or "")]
    for candidate in candidates:
        ...
        try:
            ...
            continue
    return "", str(config.get("cookie_file") or "")


def cookie_text_diagnostics(cookie_text: str, source: str) -> dict[str, Any]:
    info: dict[str, Any] = {
        "exists": bool(cookie_text.strip()),
        "size": len(cookie_text.encode("utf-8")),
        "valid": False,
        "count": 0,
        "has_auth_token": False,
        "has_ct0": False,
    ...
    if not cookie_text.strip():
        info["error"] = f"cookie source not found or empty: {source}"
        return info
    try:
        cookies, user_id, _source_format = parse_cookie_text(cookie_text)
    except Exception as error:
        info["error"] = str(error)
        return info
    names = {str(cookie.get("name") or "") for cookie in cookies}
...
def cookie_file_for_tools(config: dict[str, Any]) -> Path:
...
class BrowserCollector:
    ...
    def _load_cookies(self) -> tuple[list[dict[str, Any]], str | None]:
        cookie_text, source = load_cookie_source(self.config)
        if not cookie_text:
            raise RuntimeError(f"cookie source not found: {source}")
        cookies, user_id, source_format = parse_cookie_text(cookie_text)
        if not cookies:
            raise RuntimeError(f"cookie source is empty or invalid: {source} ({source_format or 'unknown'})")
        return cookies, user_id
...
class Downloader:
    ...
    def download_video(self, item: dict[str, Any]) -> list[str]:
        ...
        cookie_file = cookie_file_for_tools(self.config)
        command = [
            "yt-dlp",
            "--cookies",
            str(cookie_file),
...
class App:
    ...
    def cookie_summary(self) -> dict[str, Any]:
        cookie_text, source = load_cookie_source(self.config)
        return cookie_text_diagnostics(cookie_text, source)
...
<body>
  ...
  <script>
    ...
    function updateCookieSummary(summary) {
      ...
      const source = summary?.source ? "；来源 " + summary.source : "";
      $("cookieSummary").textContent = `Cookie：${ok}；数量 ${summary?.count || 0}；${auth}；${ct0}；${twid}${source}${summary?.error ? "；提示：" + summary.error : ""}`;
    }
