// ==UserScript==
// @name           XHS-Downloader2.3.1自定义下载版本
...
`,
            ...
            dockerHostLabel: 'Docker 服务地址',
            dockerHostDesc: 'NAS 或 Docker 主机 IP / 域名；可填 192.168.1.20 或 http://192.168.1.20',
            dockerPortLabel: 'Docker 服务端口',
            dockerPortDesc: 'NAS Auto Download 统一网页端口，默认 14001',
            dockerPathLabel: 'Docker 接收路径',
            dockerPathDesc: '小红书链接批量接收接口，默认 /api/xhs/links',
            ...
            extractRecommendLinksText: '提取推荐作品链接并发送',
            extractRecommendLinksDescription: '提取推荐页面的作品链接并发送至 Docker',
            downloadNoteFilesText: '发送当前作品到 Docker',
            downloadNoteFilesDescription: '把当前作品链接发送给 Docker，下载在 NAS 端继续执行',
            pushDownloadTaskText: '推送下载任务',
            pushDownloadTaskDescription: '向服务器发送下载请求',
            extractPublishedLinksText: '提取发布作品链接并发送',
            extractPublishedLinksDescription: '提取账号发布作品链接并发送至 Docker',
            extractLikedLinksText: '提取点赞作品链接并发送',
            ...
            extractSearchUsersLinksDescription: '提取搜索结果的用户链接并发送至 Docker',
            extractAlbumNotesLinksText: '提取专辑作品链接并发送',
            extractAlbumNotesLinksDescription: '提取当前专辑的作品链接并发送至 Docker',
            ...
            pushTaskSuccess: "已向 Docker 发送下载请求",
...
`,
            ...
            dockerHostLabel: 'Docker Host',
            dockerHostDesc: 'NAS or Docker host IP/domain, e.g. 192.168.1.20 or http://192.168.1.20',
            dockerPortLabel: 'Docker Port',
            dockerPortDesc: 'NAS Auto Download web port, default 14001',
            dockerPathLabel: 'Docker API Path',
            dockerPathDesc: 'Batch receiver for XHS links, default /api/xhs/links',
            ...
            extractRecommendLinksText: 'Extract and Send Recommended Note Links',
            extractRecommendLinksDescription: 'Extract recommended note links and send them to Docker',
            downloadNoteFilesText: 'Send Current Note to Docker',
            downloadNoteFilesDescription: 'Send current note link to Docker; NAS continues the download',
            pushDownloadTaskText: 'Push Download Task',
            pushDownloadTaskDescription: 'Send download request to the server',
            extractPublishedLinksText: 'Extract and Send Published Note Links',
            extractPublishedLinksDescription: 'Extract published note links and send them to Docker',
            extractLikedLinksText: 'Extract and Send Liked Note Links',
            ...
            extractSearchUsersLinksDescription: 'Extract user links from search results and send them to Docker',
            extractAlbumNotesLinksText: 'Extract and Send Album Note Links',
            extractAlbumNotesLinksDescription: 'Extract note links from the current album and send them to Docker',
            ...
            pushTaskSuccess: "Download request sent to Docker successfully",
    ...
    const defaultDockerHost = "192.168.1.20";
    const defaultDockerPort = "14001";
    const defaultDockerPath = "/api/xhs/links";
    ...
    let config = {
        ...
        dockerHost: GM_getValue("dockerHost", defaultDockerHost),
        dockerPort: GM_getValue("dockerPort", defaultDockerPort),
        dockerPath: GM_getValue("dockerPath", defaultDockerPath),
    ...
    const updateDockerHost = (value) => {
        config.dockerHost = value || defaultDockerHost;
        GM_setValue("dockerHost", config.dockerHost);
    }

    const updateDockerPort = (value) => {
        config.dockerPort = value || defaultDockerPort;
        GM_setValue("dockerPort", config.dockerPort);
    }

    const updateDockerPath = (value) => {
        config.dockerPath = value || defaultDockerPath;
        GM_setValue("dockerPath", config.dockerPath);
    }

    const dockerEndpointWithPath = (apiPath) => {
        let host = String(config.dockerHost || defaultDockerHost).trim().replace(/\/+$/, '');
        const port = String(config.dockerPort || '').trim();
        let path = String(apiPath || defaultDockerPath).trim();
        ...
        if (port && !/:\d+$/.test(host.replace(/^https?:\/\//i, ''))) {
            host = `${host}:${port}`;
        }
        return `${host}${path || defaultDockerPath}`;
    }
    ...
    const sendCurrentNoteLinkEvent = () => {
        const url = window.location.href;
        if (url.includes("https://www.xiaohongshu.com/explore/") || url.includes(
            "https://www.xiaohongshu.com/discovery/item/")) {
            showToast('正在发送当前作品链接到 Docker...');
            sendUrlsToApi([url]).then();
        } else {
    ...
    const noteUrlFromTuple = ([id, token,]) => `https://www.xiaohongshu.com/discovery/item/${id}?source=webshare&xhsshare=pc_web&xsec_token=${token}&xsec_source=pc_share`;

    const noteItemFromTuple = ([id, token, cover, author, title]) => ({
        id: String(id || ''),
        token: String(token || ''),
        author: String(author || ''),
        title: String(title || ''),
        url: noteUrlFromTuple([id, token]),
    });

    const generateNoteUrls = data => data.map(noteUrlFromTuple).join(" ");
    ...
    const AUTO_STOP_TITLE = '数学界最美味的一天';

    const applyAutoRangeForLikedAndSaved = (data, order) => {
        if (![1, 2].includes(order) || !Array.isArray(data) || data.length === 0) {
            return { data, found: false, stopIndex: -1 };
        }

        const stopIndex = data.findIndex((item) => {
            const title = item?.[4];
            return typeof title === 'string' && title.trim() === AUTO_STOP_TITLE;
        });
    ...
    const extractAllLinks = (callback, order) => {
        scrollScreenEvent(() => {
            ...
            if (autoRangeResult.found && rangeLimitedData.length === 0) {
                showToast(`已找到《${AUTO_STOP_TITLE}》，其上方没有更新内容，本次不发送任何链接。`);
                callback("", []);
                return;
            }
            ...
            if (order === 4) {
                ...
                const modalList = data.map(([id, token, cover, author, title], index) => ({
                    id, token, image: cover, author, title,
                    prechecked: autoRangeResult.found ? index < autoRangeResult.stopIndex : true,
                }));

                showListSelectionModal(modalList).then((selected) => {
                    if (selected && selected.length > 0) {
                        urlsString = generateNoteUrls(selected.map(item => [item.id, item.token]));
                        callback(urlsString, selected.map(item => ({
                            id: String(item.id || ''),
                            token: String(item.token || ''),
                            author: String(item.author || ''),
                            title: String(item.title || ''),
                            url: noteUrlFromTuple([item.id, item.token]),
                        })));
                    } else {
                        callback("", []);
                    }
                ...
                urlsString = generateNoteUrls(rangeLimitedData.map(item => item.slice(0, 2)))
                callback(urlsString, rangeLimitedData.map(noteItemFromTuple));
            }
        }, [0, 1, 2, 5].includes(order))
    };

    const extractAllNoteItems = (callback, order) => {
        scrollScreenEvent(() => {
            let data;
            if (order >= 0 && order <= 2) {
                data = extractNotesInfo(order);
    ...
    const showProgressModal = () => {
        const existed = document.getElementById('progressOverlay');
        if (existed) {
            existed.remove();
        }
        ...
        overlay.style.cssText = `
            ...
            z-index: 10001; animation: fadeIn 0.3s;
        `;
        ...
        header.innerHTML = `<span>正在发送链接到 Docker</span>`;

        const body = document.createElement('div');
        body.className = 'modal-body';

        const statusText = document.createElement('p');
        statusText.style.cssText = `font-size: 1rem; color: #333; margin-bottom: 10px; text-align: center;`;
        statusText.textContent = '准备中...';

        const progressContainer = document.createElement('div');
        progressContainer.style.cssText = `width: 100%; background: #eee; border-radius: 8px; overflow: hidden;`;

        const progressBar = document.createElement('div');
        progressBar.style.cssText = `width: 0%; height: 16px; background: #2196F3; border-radius: 8px; transition: width 0.3s ease;`;

        const logArea = document.createElement('textarea');
        logArea.readOnly = true;
        logArea.style.cssText = `
            width: 100%; height: 300px; margin-top: 15px; background: #f5f5f5;
            border: 1px solid #ddd; border-radius: 8px; padding: 10px;
            font-size: 0.8rem; line-height: 1.5; color: #444; resize: none;
    ...
    const normalizeNoteItems = (items) => (items || [])
        .filter(item => item && (item.title || item.url || item.id))
        .map(item => ({
            id: String(item.id || ''),
            url: String(item.url || ''),
            author: String(item.author || ''),
...
`;
    ...
    const sendUrlsToApi = async (urls, items = []) => {
        ...
        const endpoint = dockerApiEndpoint();
        logArea.value = `准备发送 ${total} 个链接到 ${endpoint}
`;
        logArea.value += `========================================
`;
        normalizedUrls.forEach((url, index) => {
            logArea.value += `[${index + 1}/${total}] ${url.substring(0, 140)}
`;
        });
        logArea.value += `========================================
正在等待 Docker 接收确认...
`;
        ...
        statusText.textContent = `正在发送 ${total} 个链接...`;
        ...
        try {
            ...
            const confirmed = Boolean(result.ok)
                && Number(result.valid) === total
                && Number(result.accepted || 0) + Number(result.skipped || 0) === total
                && (!result.invalid || result.invalid.length === 0);
            progressBar.style.width = confirmed ? '100%' : '70%';
            if (!confirmed) {
                throw new Error(`Docker 未完整确认：${JSON.stringify(result)}`);
            }
            logArea.value += `Docker 已确认接收。
新入队：${result.accepted}
已在队列中：${result.skipped}
...
`;
            ...
            logArea.value += `发送失败：${error.message || error}
`;
            statusText.textContent = '发送失败，请检查设置后重试';
...
`;
    ...
    const extractAllLinksEvent = (order = 0) => {
        extractAllLinks((urlsString, items) => {
            if (urlsString && urlsString.trim() !== '') {
                const urls = urlsString.trim().split(/\s+/);
                showToast(`提取到 ${urls.length} 个链接，将发送到 Docker...`);
    ...
    const showSettings = () => {
        ...
        const dockerHost = createTextInput({
                                               label: t.dockerHostLabel,
                                               description: t.dockerHostDesc,
                                               placeholder: defaultDockerHost,
                                               value: GM_getValue("dockerHost", defaultDockerHost),
                                           });

        const dockerPort = createTextInput({
                                               label: t.dockerPortLabel,
                                               description: t.dockerPortDesc,
                                               placeholder: defaultDockerPort,
                                               value: GM_getValue("dockerPort", defaultDockerPort),
                                           });

        const dockerPath = createTextInput({
                                               label: t.dockerPathLabel,
                                               description: t.dockerPathDesc,
                                               placeholder: defaultDockerPath,
                                               value: GM_getValue("dockerPath", defaultDockerPath),
                                           });
    ...
    function showListSelectionModal(list, options = {}) {
        ...
        `;
            ...
            list.forEach((item, index) => {
                ...
                checkbox.checked = typeof item.prechecked === 'boolean' ? item.prechecked : prechecked;
    ...
    const updateMenuContent = () => {
        ...
        if (!config.disclaimer) {
            ...
            menuItems.push({
                               ...
                               icon: ' 📦 ',
                               action: () => sendCurrentNoteLinkEvent(),
